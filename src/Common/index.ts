...
export const SwapTokens:any = [
  // {name:'ETH',value:WETH_ADDRESSSES},
]
export const LiquidityTokens:any = [
    // {name:'ETH',value:WETH_ADDRESSSES},
]
...
export function formatUnitsBint(s: string | number){
  if (!s) return ''
...
export function formatName(account: string | undefined, decimals = 3) {
  if (!account) {
      return ""
  }
  return account.slice(0, decimals) + '**' + account.slice(-decimals);
...
export function formatAccountEnd(account: string | undefined, decimals = 4) {
  if (!account) {
      return ""
  }
  if (account == ZERO_ADDRESS) {
